...
export class Migration20260215120000 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    // Drop localized_content only when the legacy catalog table exists.
    this.addSql(`
      do $$
      begin
        if exists (
  ...
  override async down(): Promise<void> {
    // Re-add localized_content only when the legacy catalog table exists.
    this.addSql(`
      do $$
      begin
        if exists (
    ...
    // Migrate data back only when the legacy catalog table exists.
    this.addSql(`
      do $$
      begin
        if exists (
